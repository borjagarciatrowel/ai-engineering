...
Usage (standalone, from the repo root — uv resolves the inline deps, no project venv needed)::
    ...
    #    cd estimator && docker compose up -d estimator postgres redis   # this repo: one Postgres, compose in estimator/
    #    docker compose run --rm estimator python scripts/query_examples.py   # (from estimator/)
