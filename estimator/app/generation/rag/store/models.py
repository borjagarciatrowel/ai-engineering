...
These tables register on the same ``Base`` as the Session-6 tables
(``app.foundation.persistence.models``), so a single Alembic metadata sees the
whole relational schema of the service.
"""
...
class ChunkRow(Base):
    ...
    # ``content``. The ``english`` config matches the corpus (see migration 0003).
