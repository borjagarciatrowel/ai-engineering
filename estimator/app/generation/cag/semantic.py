...
class EstimationSemanticCache:
    ...
    def delete_bucket(self, request: EstimationRequest, prompt_version: str) -> int:
        """Delete every stored entry whose bucket matches the request.

        Used when re-estimating: dropping the semantic entries for this bucket
        guarantees the changed prompt regenerates instead of being served a
        similar cached answer. Best-effort; returns the number of keys removed.

        ``redis_client`` is built with ``decode_responses=False`` so SCAN keys
        and HGET values come back as bytes.
        """
        bucket = self.bucket_for(request, prompt_version)
        deleted = 0
        try:
            prefix = _INDEX_SCHEMA["index"]["prefix"]
            for key in self.redis_client.scan_iter(match=f"{prefix}*"):
                stored = self.redis_client.hget(key, "bucket")
                if stored is None:
                    continue
                ...
                if stored == bucket:
                    ...
                    deleted += 1
            log.info("semantic_cache_bucket_deleted", bucket=bucket, deleted=deleted)
        except Exception as exc:  # noqa: BLE001
            log.warning(
