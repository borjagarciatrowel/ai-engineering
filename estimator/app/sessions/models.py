...
- ``Session`` is the in-process representation of a conversation. Unlike the
  reference exercise (which keeps these in a process-memory dict), this build
  PERSISTS each session to Postgres: ``DbSessionStore`` round-trips the model
  to/from the ``chat_sessions`` row, so memory survives a service restart and
  is shared across workers. These Pydantic models stay storage-agnostic — the
  store owns the (de)serialisation.
"""
