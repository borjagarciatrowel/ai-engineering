...
class EstimationService:
    ...
    def __init__(
        ...
        *,
        ...
        # Session 7 live: the per-role models are resolved through the runtime
        # config store (Redis) so the Settings UI can switch them mid-session.
        # When no store is wired (tests), the settings default is used verbatim.
        self._runtime_config = runtime_config
        self._initial_metadata_extractor_model = metadata_extractor_model
        self._initial_compression_model = compression_model
        self.anchor_detection_mode = anchor_detection_mode
        self._initial_critic_model = critic_model
        self.boss_max_iterations = boss_max_iterations

    def _effective_model(self, key: str, initial: str) -> str:
        """Runtime override for ``key`` if a config store is wired, else ``initial``."""
        if self._runtime_config is not None:
            return self._runtime_config.effective(key)
        return initial

    @property
    def metadata_extractor_model(self) -> str:
        return self._effective_model("METADATA_EXTRACTOR_MODEL", self._initial_metadata_extractor_model)

    @property
    def compression_model(self) -> str:
        return self._effective_model("COMPRESSION_MODEL", self._initial_compression_model)

    @property
    def critic_model(self) -> str:
        return self._effective_model("CRITIC_MODEL", self._initial_critic_model)
    ...
    def invalidate_caches(self, request: EstimationRequest) -> None:
        ...
        Used by the re-estimation flow: after the user edits the prompt we must
        not serve the previous answer from the exact or the semantic cache.
        """
    ...
    def estimate(self, request: EstimationRequest) -> EstimationResponse:
        ...
        # 8. Return with per-call telemetry attached.
        usage = LlmUsage(
