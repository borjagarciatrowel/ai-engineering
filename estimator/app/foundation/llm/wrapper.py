...
- ``primary_model``/``fallback_model`` are PROPERTIES backed by the runtime
  ...
  as a per-call ``model_override``. (Session 7 live.)
"""
...
class LLMWrapper:
    ...
    # ------------------------------------------------------------------
    # Effective models (runtime override > settings default) — Session 7 live
    # ------------------------------------------------------------------
    ...
    def complete_structured(
        ...
        *,
        ...
        try:
            ...
            # completion, so we can read token usage (instructor's plain
            # ``create`` discards it). Note: on a re-prompt loop this is the
            # usage of the FINAL successful call, not the sum of all attempts.
