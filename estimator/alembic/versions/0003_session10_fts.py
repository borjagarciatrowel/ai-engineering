...
ENGLISH ("Mobile banking API", "Headless e-commerce storefront", …). We therefore
use the ``english`` configuration: its stemming and stop-word list are what
actually lift lexical recall on this corpus. Swapping to ``spanish`` would only
change the regconfig string in two places (here and the ``plainto_tsquery`` call
in ``repository.search_lexical``) — both kept consistent on purpose.
