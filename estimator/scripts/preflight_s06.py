...
Adapted from the professor's reference to OUR stack: we use the psycopg2 driver
(``psycopg2-binary``) and we do NOT ship ``unstructured`` (the XLSX/DOCX/PDF
parsers that would use it live in the instructor's guides/, not here), so those
two checks differ from the reference.
...
# Our stack: psycopg2-binary (not psycopg v3) and NO unstructured.
_MIN_VERSIONS = {
    "pandas": "2.2.0",
    "pandera": "0.20.0",
    "pyyaml": "6.0",
