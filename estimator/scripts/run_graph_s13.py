...
Persistence + observability (Level 2):
...
``--stub`` swaps the real S9/S10 retrieval for the offline reference stub already
ported in ``exercises/session-12/reference_retrieval.py`` so ``search_budgets``
needs no database. The real path needs the historical-task corpus ingested
(``scripts/build_task_corpus.py --ingest``). The default transcript is the S12
complex transcript, reused as-is rather than duplicated (see the exercise doc).
"""
