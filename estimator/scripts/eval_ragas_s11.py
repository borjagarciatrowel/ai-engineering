...
Prerequisites (host run):
    ...
    * dev deps installed: uv sync --group dev  (pulls ragas + datasets)

ragas 0.4.x unconditionally imports ``langchain_community.chat_models.vertexai``
at module load, which this project's ``langchain-community`` does not expose;
``run_ragas`` stubs that import (see ``scripts/score_ragas_s11.py``) before
importing ragas, so collection and scoring run in the SAME venv by default.
``--collect-only``/``--score-file`` remain available for the rare case where
the two dependency trees genuinely cannot coexist (e.g. a future langchain
bump that ragas cannot tolerate at all) — collect in the project venv, then
score the dumped samples with ``scripts/score_ragas_s11.py`` in an isolated one.
...
def run_ragas(samples: list[dict], judge_model: str, embedding_model: str):
    ...
    # ragas 0.4.x unconditionally imports langchain_community.chat_models.vertexai
    # at module load; this project's langchain-community (0.4.2) does not expose
    # it. We never use Vertex (the judge is OpenAI), so stub the import instead
    # of pulling the Vertex SDK. See scripts/score_ragas_s11.py for the isolated-
    # venv alternative when the two dependency trees genuinely cannot coexist.
...
def score_samples(samples: list[dict], judge_model: str, embedding_model: str, out: str | None):
    ...
    langchain stack can conflict with ragas's hard vertexai import.
    """
