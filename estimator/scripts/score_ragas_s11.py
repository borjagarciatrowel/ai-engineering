...
so it can run in an isolated venv, decoupled from the project's langchain
stack (some ragas releases hard-import ``langchain_community.chat_models.vertexai``,
which recent ``langchain-community`` releases may not expose). Collect the
samples first with::
...
then score them here::
    ...
    uv run python scripts/score_ragas_s11.py samples.json --out report.json

We never use Vertex (the judge is OpenAI); if the Vertex import path is
missing, lightweight stub modules are registered before importing ragas so
the import succeeds without pulling the Vertex SDK.
"""
