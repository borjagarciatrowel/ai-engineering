...
(same pattern as the integration tests, DB-backed via ``sqlite_db``), drive a
couple of cases through the real endpoint, and assert metrics flow through.
Real LLM is never called.
"""
...
def test_run_against_fake_wrapper_collects_metrics_per_case(sqlite_db, tmp_path: Path) -> None:
