...
Persistence note (project deviation): the in-process mode points the app's
``get_db`` at a throwaway in-memory sqlite so the eval run is fully
self-contained — it neither needs a running Postgres nor leaks into the dev
server's ``chat_sessions``. The real DbSessionStore runs unchanged on top.
"""
...
def _install_sqlite_db():
    """Point the app's ``get_db`` at a throwaway in-memory sqlite for the run.

    Returns the override callable so ``main`` can pop it afterwards. A single
    shared engine (StaticPool) keeps POST /sessions and POST /sessions/{id}/...
    landing in the same database across requests.
    """
...
def main() -> int:
    ...
    # Isolated in-memory sqlite session storage for the eval run so we don't
    # touch Postgres or leak into sessions the dev server may also be using.
    # HTTP mode talks to a separate process, so the override is a no-op there.
    _install_sqlite_db()
