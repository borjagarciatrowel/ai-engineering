...
> **Divergencias de este repo (frente a la referencia del profesor).**
>
> - **Frontend Angular, no Rails.** La solución de referencia añade además un CRUD Rails
>   (`supervisor_estimation_runs`: bandeja de revisión, badge de estado, panel de traza de enrutado y
>   de auditoría) sobre `estimator-web`. Aquí **no se porta**: nuestro frontend es Angular
>   (`estimator-frontend`) y el contrato HTTP del servicio IA es idéntico, así que la UI se puede
>   construir después contra los mismos tres verbos sin tocar nada de este paquete.
> - **Corpus más grande.** Nuestro corpus histórico está ingerido sobre el Postgres del proyecto
>   (1.603 `budget_chunks` / 75 documentos), así que la transcripción edge case **encuentra análogos
>   para casi todos sus componentes**. Ver el aviso sobre el umbral más abajo: no es un detalle
>   cosmético, cambia cuándo salta la puerta.
> - **Un solo Postgres.** Checkpointer, pgvector y datos de producto conviven en la misma base, como
>   en el resto de sesiones de este repo.
...
**Ejecutado y verificado en este repo** (API en el host con `uv run uvicorn`, Postgres del
`docker-compose` con el checkpointer real, `SUPERVISOR_CONFIDENCE_THRESHOLD=0.7` — ver el aviso del
umbral más abajo):

| Paso | Resultado observado |
|---|---|
| START | `state=paused`, `status=awaiting_human_review`, `reasons=["confidence 0.41 is below the 0.70 threshold"]`, `routing_history` = 5 decisiones, todas `source=llm`, 17 filas de auditoría |
...
| RESUME (`adjust`) | `state=completed`, `status=validated`, `human_decision` plegado en el estado, auditoría 17 → **18** filas (la fila `human/review_decision`; **cero duplicados**, el reducer keyed aguanta la reejecución del nodo) |
...
| `example_run_edge_case.txt` | La traza comprometida del entregable. En este repo está generada con la **ejecución real**: recuperación pgvector sobre el corpus ingerido (latencias de 175–272 ms por búsqueda, no 0 ms) y checkpointer `AsyncPostgresSaver`. La puerta saltó de verdad (`confidence 0.28 < 0.60`, dos componentes sin precedente) y se reanudó. |
...
> **Aviso honesto sobre el umbral con NUESTRO corpus (divergencia real, medida).** Con el corpus
> ingerido de este repo la transcripción edge case **ancla todos sus componentes** (9 componentes,
> 41 análogos en una de las ejecuciones). Y ahí aparece un límite del cálculo de confianza que
> conviene ver:
>
> ```
...
> ```
>
> El umbral por defecto es **0.60** y la comparación es estricta (`<`), así que una estimación
> perfectamente anclada cuya etiqueta del modelo sea `medium` cae **exactamente sobre el umbral y no
> pausa**. Verificado dos veces por HTTP contra el stack real: `status = "validated"`,
> `confidence = 0.6`. Solo pausa si el modelo se autoetiqueta `low`, si algún componente queda sin
> análogo, o si los guardarraíles levantan issues — que es lo que ocurre en la traza comprometida
> (`confidence 0.28`, dos componentes sin precedente).
>
> No es un bug: es la puerta haciendo su trabajo con un corpus que sí tiene precedentes. Pero
> significa que **con nuestro corpus el umbral útil está por encima de 0.60**. El paseo HTTP de abajo
> se ejecutó con `SUPERVISOR_CONFIDENCE_THRESHOLD=0.7` — la perilla que el propio enunciado señala
> como configurable — y la puerta saltó con `confidence 0.41`.
